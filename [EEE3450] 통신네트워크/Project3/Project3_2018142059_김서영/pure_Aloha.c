...
#define Slot (1000 * MINI_Slot)  //sec
...
{   int N;
	...
	while(N <= 16)
	{
		...
		{   p=(totalRate)/N;  //probability that a node will transmit a frame 
			channel  = IDLE;
			...
			while(tick < SIMTIME)	//Time slots
			{
				for(int i=0; i < N; i++)
				{
					...
					{
						channel = TRANSMIT;
						txEndTime = tick + Frame_LENGTH;
					}

					else if(frame_transmitted==1)
					{
						channel = COLLISION;
						txEndTime = tick + Frame_LENGTH;
					}
			...
			fprintf(fp, "(%.6f %.8f)\t(%.8f %.8f)\n",
					totalRate * Slot,
					totalRate * Slot * exp(-2 * totalRate * Slot),
