...
void mySender(int sock)
{
	...
	while (!sndPkt.isFinish)
	{
		...
		fSize = fSize - sndPkt.dataSize;
		sentSize = sentSize + sndPkt.dataSize;
		sndPkt.dataSeq = sndPkt.dataSeq + 1;
		sndPkt.indicator = sndPkt.indicator^1;
		usleep(500000);
	}
...
void rdtSend(int sock, Packet pkt)
{
	...
	// retramsmit packet when Timeout occurs
    	...
    	while(!isACK(sock,pkt.indicator))
	    {
...
bool isACK(int sock, int idc)
{
	...
	while (timer_count++ < TIMEOUT)
	{
		...
		read(sock, &ACK, sizeof(ACK));  //Use read()

		if (ACK.indicator == idc)
